...
class RR3d_MIL(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = nn.Sequential(
            nn.ReLU(True),
            nn.Dropout(p=0.5),     
            nn.Linear(block_inplanes[3] *self.K*self.J,block_inplanes[3]),
            nn.Dropout(p=0.5),  
        )

        self.bag_classifier = nn.Linear(block_inplanes[3], n_classes)
    ...
    def forward(self, x_in, x_adc, x_b500, x_ap, x_pvp, x_dp, x_t1, x_t2):
        ...
        out, out_f, alpha = self.attention_layer(x_patch_com, self.bag_classifier.weight, self.bag_classifier.bias, int(h*w/ps/ps))
        ...
        bag_preds = self.bag_classifier(out)
        return bag_preds, out_f, alpha


def generate_deepganet(model_depth, **args):
    assert model_depth in [10, 18, 34, 50, 101, 152, 200]

    if model_depth == 10:
        model = RR3d_MIL(BasicBlock, [1, 1, 1, 1], get_inplanes(), **args)
    elif model_depth == 18:
        model = RR3d_MIL(BasicBlock, [1, 2, 2, 2], get_inplanes(), **args)
    elif model_depth == 34:
