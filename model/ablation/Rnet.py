...
class RR3d_MIL(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1_resbase = nn.Conv3d(n_input_channels,
                               self.in_planes,
                               kernel_size=(conv1_t_size, 3, 3),
                               stride=(conv1_t_stride, 1,1),
                               padding=(conv1_t_size // 2, 1,1),
                               bias=False)
        self.bn1_resbase = nn.BatchNorm3d(self.in_planes)
        self.relu_resbase = nn.ReLU(inplace=True)
        self.maxpool_resbase = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
        self.layer1_resbase = self._make_layer(block, block_inplanes[0], layers[0],
                                       shortcut_type)
        self.layer2_resbase = self._make_layer(block,
                                       block_inplanes[1],
                                       layers[1],
                                       shortcut_type,
                                       stride=2)
        ...
        self.modal_attention = nn.Sequential(
            nn.Linear(block_inplanes[3] * block.expansion*8, 512),
            nn.ReLU(),
            nn.Linear(512, 128),
            nn.Tanh(),
            nn.Linear(128, 64)
        ...
        self.bag_classifier = nn.Linear(block_inplanes[3] * block.expansion, n_classes)
    ...
    def forward(self, x_in, x_adc, x_b500, x_ap, x_pvp, x_dp, x_t1, x_t2):
        ...
        x_resbase = torch.cat((x_in, x_adc, x_b500, x_ap,x_pvp,x_dp,x_t1,x_t2),dim=1).permute(0,1,4,2,3)

        x_resbase = self.conv1_resbase(x_resbase)
        x_resbase = self.bn1_resbase(x_resbase)
        x_resbase = self.relu_resbase(x_resbase)
        if not self.no_max_pool:
            x_resbase = self.maxpool_resbase(x_resbase)

        x_resbase = self.layer1_resbase(x_resbase)
        x_resbase = self.layer2_resbase(x_resbase)
        x_resbase = self.layer3(x_resbase)
        x_resbase = self.layer4(x_resbase)
        x_resbase = self.avgpool(x_resbase).view(b,-1)

        bag_preds = self.bag_classifier(x_resbase)
        return bag_preds


def generate_rnet(model_depth, **args):
