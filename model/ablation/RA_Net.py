...
class RR3d_MIL(nn.Module):
    ...
    ################_compose###################
        ...
        x_compose = self.layer3(x_compose)
        x_compose = self.layer4(x_compose)
        ...
        x_compose, R_patch_com, alpha_compose = self.attention_layer(x_compose, self.bag_classifier.weight, self.bag_classifier.bias, gamma)
       
        x_compose = x_compose.transpose(2,1).sum(2)
        x_compose = x_compose.view(x_compose.size(0),-1)

        bag_preds = self.bag_classifier(x_compose)
        return bag_preds, R_patch_com, alpha_compose


def generate_ranet(model_depth, **args):
